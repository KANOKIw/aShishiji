...
function startLoad(message){
    if ($(cssName.lospare).css("background-color") == "transparent")
        $(cssName.lospare).css("background-color", "white");
    $(cssName.app).show();
    $("#loaders").removeClass("bye");
    $(cssName.fselector).hide();
    $(cssName.ov_tribe).hide();
    $(cssName.lospare)
 ...
 * @param {number} [delay] 
 */
function endLoad(message, delay){
    function i(){
        $("#map__opnner").show();
        ...
        setTimeout(() => {
            ...
            $(cssName.ov_tribe).addClass("hello").show();
        }, 1500);
    }
