...
document.getElementById("inv_joiner_room")?.addEventListener("click", function(){
    const tm = `/test/?art=${passed_orgname}&storm=navigate`;
...
-async function(){
    ...
    switch(went){
        case "unstoppable":
            ...
            .then(
                /** @param {import("../shishiji-dts/objects").mapObj} orgdata */
                orgdata => {
                    if (!orgdata || !LOGIN_INFO.data.completed_org.includes(passed_orgname)){
                        $("#msg-res").text("何事...?");
                        return;
                    }
