...
import { OrgAuthColumn } from "../../server-dts/server";
import { OrgAuth } from "./auth";
import { returnError, orgDB } from "./doubt";
...
export class Cloud{
    ...
    static getOrgMaCloudSize(orgname: string): Promise<number>{
        return new Promise(function(resolve, reject){
            orgDB.all(`SELECT * FROM Auth_Data WHERE org_name=?`, [ orgname ], (err, rows: OrgAuthColumn[]) => {
                if (rows && rows[0]){
