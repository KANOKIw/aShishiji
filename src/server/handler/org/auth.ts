...
import { returnError, orgDB, random } from "./doubt";
...
export class OrgAuth{
    ...
    async data(req: Request, res: Response): Promise<void>{
        const alldata = mapObjAPI.getAllObjects(false);
        const orgname = req.body.target || "";

        res.send(alldata[orgname]);
    }
    ...
    static async _auth(session?: string): Promise<string | null> {
        return new Promise((resolve, reject) => {
            ...
            orgDB.all(`SELECT * FROM Auth_Sessions WHERE sessionid=?`, [ session ], function(err: Error | null, rows: OrgAuthSessionColumn[]) {
    ...
    static async _cidAuth(org_name: string, cid: string): Promise<string>{
        return new Promise((resolve, reject) => {
            orgDB.all(`SELECT * FROM Auth_Data WHERE org_name=?`, [ org_name ], function(err: Error | null, rows: OrgAuthColumn[]) {
